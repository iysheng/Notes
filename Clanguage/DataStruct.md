...
9. 在 gcc 中使用 asm 内联汇编的方法: **asm**("nop");
...
- `sscanf(buffer, "%1[a-z|-] %d %d", &c, &x, &y);` 这里的 `%1[a-z|-]` 表示匹配一个的 `a-z` 或者 `-` 字符
...
15. `execlp()` 函数执行指令的命令
    ...
    if (pid == 0) {
        // 子进程：尝试执行 ffmpeg
        // 重点是结尾以 NULL 结尾
        execlp("ffmpeg", "ffmpeg", "-y", "-i", rtsp_url.c_str(),
        "-c:v", "copy", "-f", "mp4", "-t", "3", "/tmp/111/aaa.mp4", (char\*)NULL);

        // 如果 execlp 返回，说明失败
        _exit(1);                 // 这个重要，立即退出子进程，返回 1 给父进程

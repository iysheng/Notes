...
2. scp 和 ssh sshpass 命令
    ...
    7. ssh -p xx user@ip /_ ssh -p 参数连接指定的端口 xx _/

3. git 使用笔记

    1. git log --oneline /_ 每一个 commit 只显示一行 _/
        - git log --pretty=oneline /_ 每一个 commit 只显示一行 _/
        - git log --pretty=ref xxx /_ 查看指定 xxx 的提交，并且显示修改日期 _/
        - git log --pretty=ref -n=yy xxx /_ 查看指定 xxx 的提交，并且显示修改日期，显示 yy 条数据 _/
    2. git log -nx /_ 显示最近的 x 次提交 _/
    3. git help --web [command] /_ 在浏览器查看 git [command] 的 help 信息 _/

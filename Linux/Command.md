1. Fedora 添加库和禁止、使能库
    ...
    16. `sudo dnf install kernel-devel-matched kernel-headers` # 安装内核开发头文件
...
3. git 使用笔记
    ...
    91. ``git archive --format=zip   -o ../release.zip HEAD`` # 以 zip 方式打包当前分支
    92. ``git archive -o ../release.tar.gz v1.0`` # 以默认 tar.gz 方式打包制定分支
    93. ``git archive --format=tar   -o ../release.tar   HEAD`` # 以 tar 方式打包
...
7. ls -lt 默认是按照文件的修改时间排序的，相应的 ls -ltu 是按照文件的访问时间排序， ls -ltc 是按照文件的 i 节点的修改时间排序
    1. ls -lhL L 选项如果是软连接，会跟随软连接查看目标文件大小
...
115.    mermaid 可以通过语句绘制漂亮的 diagram， typora 可以使用 mermaid 类型的代码块来绘制
116.    weston 知识
...
165.    使用 tr 和 dd 命令配合,生成指定大小全 0XFF 的文件 : `tr '\000' '\377' < /dev/zero | dd of=filename bs=1024 count=1`
        ...
        - ``dd if=/dev/zero bs=512 count=1 2> /dev/null | tr '\0' '\132' > test_5a.bin`` 另一种方式生成指定文件内容指定大小的文件
